...
from . import econ
from .data import SCENARIOS, ZONES, days_between

MONTH = (date(2023, 9, 1), date(2023, 9, 30))
    ...
    return {
        ...
        "by_zone": {
            z: {p: {"net": round(x.net, 2), "min_soc": round(x.min_soc, 3),
                    "kwh_out": round(x.kwh_discharged, 1), "wear": round(x.wear, 2),
        ...
        "soc": {p: [round(s.soc, 3) for s in r.by_zone["LZ_HOUSTON"][p].steps]
                for p in ("scheduled", "wattgap")},
    }
...
def month_summary() -> dict:
    ...
    cont = {p: {z: econ.simulate(p, z, days) for z in ZONES} for p in ("scheduled", "wattgap")}
...
def main() -> None:
    ...
    print(f"{'day':<24}{'naive':>9}{'fair sched':>12}{'WattGap':>10}{'gap vs fair':>13}   ($/home, avg of 4 zones)")
    for name, d in rep["days"].items():
        p = d["per_home"]
        print(f"{name + ' ' + d['day']:<24}{p['naive_overnight']:>9.2f}{p['scheduled']:>12.2f}"
              f"{p['wattgap']:>10.2f}{d['gap_vs_fair']:>+13.2f}")
    m = rep["month"]
    print(f"{'Sep 2023 (30 days)':<24}{'':>9}{m['per_home']['scheduled']:>12.2f}{m['per_home']['wattgap']:>10.2f}"
          f"{m['gap_vs_fair']:>+13.2f}   won {m['days_won']}/{m['days']} days, lowest SoC {m['min_soc']:.0%}")
    z = rep["zone_finding"]
